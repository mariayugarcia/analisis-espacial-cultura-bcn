...
write.csv(tabla_indicadores, "final/outputs/tabla_05_indicadores_barrio.csv", row.names = FALSE)

# Tabla HTML formateada

tabla_html <- knitr::kable(tabla_indicadores, 
                           caption = "Tabla 5. Indicadores territoriales y clasificación de oportunidades por barrio",
                           format = "pipe", align = "c", digits = 2) %>%
  kableExtra::kable_styling(latex_options = "striped")


writeLines(tabla_html, "final/outputs/tabla_05_indicadores_barrio.html")

# Tabla 6: Resultados de autocorrelación espacial
tabla_moran <- tibble(
...
ggsave("final/outputs/figura_07_lisa_clusters.tiff", mapa_lisa,
...
ggsave("final/outputs/figura_08_oportunidades_negocio.tiff", mapa_negocio,
       width = 10, height = 7, dpi = 300, bg = "white", compression = "lzw")
...
ggsave("final/outputs/figura_09_moran_scatter.tiff", moran_scatter,
       width = 9, height = 6, dpi = 300, bg = "white", compression = "lzw")

# Tabla 7: Barrios significativos (p-value < 0.05)
...
tabla_barrios_html <- knitr::kable(
  ...
  caption = "Tabla 7. Barrios con autocorrelación espacial significativa (p < 0.05)",
  format = "pipe",
  align = c("c", "c", "c", "c", "c"),
  digits = 4
) %>%
...
writeLines(tabla_barrios_html, "final/outputs/tabla_07_barrios_significativos.html")
write.csv(barrios_significativos, "final/outputs/tabla_07_barrios_significativos.csv", row.names = FALSE)

...
writeLines(tabla_html_categorias, "final\\outputs\\tabla_01_dffinal_cat.html")
writeLines(tabla_html_tipo, "final\\outputs\\tabla_02_dffinal_tipo.html")
...
write.csv(tabla_cobertura, "final/outputs/tabla_00_cobertura_fuentes.csv", row.names = FALSE)
...
ggsave("final\\outputs\\figura_00_cobertura_fuentes.tiff", 
       plot = p_cobertura, 
       width = 10, 
       height = 7, 
       dpi = 300, 
...
# Mapa estático
mapa_base <- ggplot() +
  geom_sf(data = bcn_muni, fill = "#f4f4f4", color = "grey50", linewidth = 0.5) +
...
ggsave("final\\outputs\\figura_01_mapa_integrado.tiff", 
       plot = mapa_base, 
       width = 10, 
       height = 7, 
       dpi = 300, 
